...
export default function ParticipantPage() {
    ...
    const quizStateIntervalRef = useRef<NodeJS.Timeout | null>(null);
    const questionTimerRef = useRef<NodeJS.Timeout | null>(null);
    const questionPollingRef = useRef<NodeJS.Timeout | null>(null);

    // --- API Call Functions ---
    const fetchQuizAndParticipantData = async () => {
        try {
    ...
    useEffect(() => {
        ...
        questionPollingRef.current = setInterval(fetchQuizAndParticipantData, 2000);
                        ...
                        {currentQuestion.options.map((option, index) => (
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-lg font-medium transition-colors duration-300
                                    ${selectedOption === option ? 'bg-orange-600 text-black' : 'bg-gray-200 hover:bg-gray-300'}
